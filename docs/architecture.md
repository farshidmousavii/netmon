...
Bidar already exists as a CLI tool (formerly netmon): `monitor` (ping + SNMP health check), `backup`/`diff` (config backup and comparison), `exec` (bulk SSH command execution), and `init` (YAML/CSV config setup). Its config-management TUI was removed during Phase 2 — the Phase 5 REST-API-client TUI will be the only one the project has. This project merges into that repository rather than living alongside it as a second tool. A full codebase audit (2026-08-07) informed the decisions below — see `roadmap.md` §Phase 0 for the resulting task list.

**What merges:**
- One `bidar` binary, one CLI (cobra — already in use). Existing subcommands (`monitor`/`backup`/`exec`/`diff`/`init`) are unchanged in behavior. The old config-file TUI was briefly renamed `bidar tui-config` (Phase 0) and later **removed entirely** — it managed `config.yaml` and held SSH credentials in-process, which is a fundamentally different tool from Phase 5's read-only REST-API-client TUI, and maintaining both indefinitely wasn't worth it once that was clear. `config.yaml`/`devices.csv` remain editable by hand or via `bidar init` for anyone still using the legacy CLI path. New subcommands are added: `bidar serve` (runs the daemon — Phases 1–7), `bidar migrate` (DB migrations), `bidar import-devices` (one-time import of an existing `config.yaml`/`devices.csv` into `network_devices`/`snmp_profiles`/`ssh_credentials`).
...
- `network_devices` (new, Postgres) becomes the canonical device list. The existing YAML/CSV config format remains a supported input via `bidar import-devices` (strictly one-directional, file → DB — it never rewrites `config.yaml`) and stays available as-is for `monitor`/`backup`/`exec`.
...
- `exec` and `backup` are **read-write**: they SSH into devices and can change configuration. The daemon (Phases 1–7) is **read-only by design** — SNMP reads, ARP reads, DHCP lease reads, LDAP reads, agent reports. It never sends a device a command that changes anything.
- Credentials follow the same split: `snmp_profiles` (read-only community/v3 keys) is separate from `ssh_credentials` (read-write, used only by `exec`/`backup`). The daemon never touches `ssh_credentials`, and daemon code (`internal/providers/*`) must never import `internal/device` (the existing SSH/exec package) — verified one-directional today (`internal/device` imports `internal/snmp`; the reverse is not true, and must stay that way).
...
- **⚠️ Back up `BIDAR_MASTER_KEY` — this is not optional.** It's the only thing that decrypts every stored secret (`snmp_profiles`, `ssh_credentials`, `dhcp_sources.credential_enc`). Lose it and those rows are permanently unrecoverable — not "hard to recover," genuinely gone, since that's the whole point of encryption. This already happened once during Phase 1 testing (an `.env` regeneration silently destroyed the key, forcing a full re-bootstrap from `import-devices` plus re-entering every credential by hand). Store it in a password manager or secrets vault the moment it's generated, separately from `.env` itself — `.env` on the host is not a backup.
...
2. **DHCP lease collector — MikroTik only in Phase 1.** `dhcp_sources.type = 'mikrotik'` calls into the shared `internal/providers/mikrotik` RouterOS API client package — the same package Phase 2 later extends with ARP/wireless-registration/interface polling. There is exactly one MikroTik client in the codebase, used narrowly here (DHCP leases only) and more fully from Phase 2 on — never two separate implementations talking to the same device.

   **Windows DHCP lease collection was built (a WinRM live-query design and a file-export design, both documented and implemented at one point) and then deliberately removed.** Both methods added real setup complexity — service-account/group provisioning for WinRM (compounded by DHCP-on-a-DC not having a local SAM, and `DHCP Users` often not being provisioned at all), or a PowerShell Scheduled Task + SMB share mount + UNC-path translation for file export — for a payoff that turned out to be replaceable more simply: see the ICMP collector below. `type = 'windows'` in `dhcp_sources` is intentionally unimplemented — same treatment as `type = 'cisco'` (§Backlog): a clean, explicit error if anyone configures one, not a silent no-op, and not a speculative re-build without a real deployment that actually needs it.

3. **ICMP sweep (fallback/freshness layer, and — with a DNS-integrated DHCP setup — a hostname source too)** — periodic ping sweep of configured `subnets`. Cheapest and fastest way to refresh "still up" between ARP snapshots; also catches hosts that don't yet have a fresh ARP entry. When a host answers, the collector also does a reverse-DNS (PTR) lookup on its IP — if the DHCP server dynamically registers DNS records for its clients (a standard Windows DHCP setting, already enabled in this environment), this recovers the hostname evidence the Windows DHCP collector would have provided, with zero server-side setup. This is why removing Windows DHCP collection was viable rather than a functionality loss: ARP already provides MAC+VLAN, ICMP+rDNS now provides IP+hostname+liveness, and MikroTik DHCP remains available for the source type where it was actually simple to support.
...
### Design
- `bidar tui` — the *only* TUI in the project by the time this phase runs, built with `bubbletea`/`lipgloss` (already in `go.mod` — check whether they're still a dependency after the config-file TUI's removal, or need re-adding here). The earlier config-management TUI (`tui-config`) was removed entirely rather than kept alongside this one.
...
1. **Network-presence mechanism — resolved**: not a single mechanism. Three collectors run in parallel: SNMP ARP reads from *all* core/L3 switches, DHCP lease reads from configured MikroTik sources, and ICMP sweep (with reverse-DNS enrichment) as a fast freshness/hostname fallback. See §Phase 1 above.
2. **Agentless desktop inventory (WMI/DCOM/WinRM from a central server) — considered and rejected, in favor of Phase 6's per-endpoint Agent.** Tools like EMCO/Lansweeper/PDQ that market themselves as "agentless" still work this way: one central credential with admin rights over the *entire* desktop fleet, used continuously by an always-on server. That's a materially different risk shape than Phase 6's agent — a compromised central server/credential in the agentless model means fleet-wide admin exposure, versus each endpoint reporting read-only about itself with the agent model. This is the same Option A/B rejection from the original design review (agentless WMI/WinRM), reaffirmed here for the same reasons plus this security framing. Not revisited unless a deployment has a concrete reason the agent genuinely can't work.
