...
- **Windows DHCP Method A (WinRM live query)** — documented in `architecture.md` §Phase 1 but deliberately not implemented. Build it if a future deployment has Windows DHCP on a non-DC member server (where the risk profile and `DHCP Users`/`Remote Management Users` group setup are more straightforward) and actually needs live queries instead of the file-export method.
...
6. `internal/providers/dhcp`: pluggable per-`source_type` lease collector, iterating every enabled row in `dhcp_sources`, write to `host_observations`. For `source_type = mikrotik`, build `internal/providers/mikrotik` as a real RouterOS API client package now (not a throwaway) — Phase 2 extends it rather than replacing it. For `source_type = windows`: **implement Method B (file export) only** — read leases from a JSON file at a configured filesystem path (`connection_config.path`), produced by an org-run scheduled task using `scripts/export-dhcp-leases.ps1` (new, added this task). Do not implement Method A (WinRM) — see `architecture.md` §Phase 1 for why it's deferred until a deployment actually needs it.
