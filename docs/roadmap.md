...
- ~~**Verify the `ParseVendorSNMP` fix against a live device capture.**~~ — resolved during the Phase 2 bake-in: real responses captured from a Cisco Catalyst (`.1.3.6.1.4.1.9.1.1178`) and a MikroTik RouterOS box (`.1.3.6.1.4.1.14988.1`) both parse correctly through the leading-dot normalization.
- ~~gofmt drift in three pre-existing legacy files (`backup/diff.go`, `logger/logger.go`, `report/json.go`)~~ — resolved, cleaned up in its own isolated commit alongside the Windows-DHCP-removal work.
...
- **Windows DHCP collection (both methods)** — documented in `architecture.md` §Phase 1 but not present in the running system. Method A (WinRM live query) was never built. Method B (file export: PowerShell script, SMB share mount, UNC-path translation) _was_ built, then deliberately removed once ICMP+rDNS proved a simpler replacement for the hostname evidence it provided — see `architecture.md` §Phase 1 for the reasoning. Either could be revisited for a deployment where MikroTik-only DHCP + ARP + ICMP/rDNS genuinely isn't enough (e.g. no DNS integration on the DHCP server), but build against real need, not speculatively.
- **RouterOS API-SSL (encrypted transport)** — `internal/providers/mikrotik`'s client currently talks the plain (unencrypted) RouterOS API on port 8728; credentials cross the LAN in cleartext. Fine for a trusted management segment, but revisit (switch to `api-ssl` on 8729 with certificate validation) before any deployment where that segment isn't fully trusted.
- **Cisco IOS as a DHCP source — currently unsupported, real gap.** Unlike MikroTik (RouterOS API gives lease data directly) or Windows (file export), Cisco has no clean SNMP-exposed DHCP binding table — the only real source is `show ip dhcp binding` over SSH, which is exactly the channel `internal/providers/*` must never touch. Supporting this properly would need a new, narrowly-scoped read-only SSH client (show-commands only, nothing like `exec`'s general command execution) — don't build it speculatively; design it if/when a deployment actually needs Cisco-as-DHCP-server support. `dhcp_sources.source_type = 'cisco'` should be treated as explicitly unimplemented (clean error, not silent skip) if anyone tries to configure it before this is built.
- **SNMPv3 + per-VLAN MAC tables (Cisco BRIDGE-MIB) — known gap.** The `community@vlan` trick that gets per-VLAN MAC tables out of Cisco switches is inherently an SNMPv2c mechanism; there's no v3-native equivalent without SNMPv3 context-name support, which `internal/snmp` doesn't have. A v3-configured device currently falls straight to the Q-BRIDGE-MIB fallback (works on switches that support it without per-VLAN contexts; silently gets less complete MAC-table data on switches that need the per-VLAN walk and are v3-only). No impact today (this deployment's profile is v2c), but a real limitation for a future v3-only deployment — revisit if that comes up.
...
6. `internal/providers/dhcp`: `source_type = mikrotik` only — build `internal/providers/mikrotik` as a real RouterOS API client package (not a throwaway); Phase 2 extends it rather than replacing it. `source_type = windows`/`cisco` return a clear "unimplemented" error if configured, never a silent no-op. **Remove the previously-built Windows file-export path entirely** (`scripts/export-dhcp-leases.ps1`, the compose SMB-mount/`DHCP_SHARE_SRC` wiring, the UNC-path translation logic in `dhcp-sources set-path`, and the `windows`-specific validation) — it was built, tested, and then deliberately dropped once ICMP+rDNS (task 7) proved sufficient; don't leave the dead code/docs/compose config behind.
7. `internal/providers/icmpsweep`: ping sweep of enabled `subnets`, write to `host_observations`. **Also**: on a successful ping, do a reverse-DNS (PTR) lookup on the IP and record the resolved hostname on the observation (best-effort — a failed/empty PTR lookup is not a provider failure, just missing evidence). This is what replaces the Windows DHCP hostname evidence removed from task 6 — relies on the DHCP server dynamically registering client DNS records, confirmed already enabled in this environment; document that dependency clearly since it won't hold for every deployment.
...
4. ~~`bidar dhcp-sources set-path <name> <path>`~~ — **removed** along with the Windows DHCP collection it existed for (Phase 1 task 6 revision): with `windows` unimplementable, no Phase 1 source type uses a file path, so the command could only ever error. The store-level `SetDHCPSourcePath` went with it.
5. `README.md`: a real "Quick Start" section using only `bidar` subcommands end-to-end (`import-devices` → `devices list`/`set-role` → `dhcp-sources add` → `docker compose up`) — zero raw SQL in the documented path. Direct DB access remains available for debugging, just not part of the onboarding story anymore.
...
- `bidar devices set-role` and `bidar dhcp-sources add` take effect on the next scheduled poll cycle without restarting `bidar serve`.

**Status: complete.** Both DoD bullets verified live against the running stack (role change picked up by the ARP provider's next cycle with zero restart; `README.md`'s Quick Start is now `psql`-free end to end).
