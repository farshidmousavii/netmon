...
4. `internal/providers/ad`: LDAP(S) bind, pull computer objects, write to `host_observations`, upsert into `hosts` via matching rules (see `architecture.md` §Phase 1). Env vars: `BIDAR_AD_URL` (ldap:// or ldaps:// endpoint), `BIDAR_AD_BIND_DN`, `BIDAR_AD_BIND_PASSWORD` (read-only service account; the operator does not share domain-admin accounts), `BIDAR_AD_BASE_DN` — pinned here per AGENTS.md §6.
